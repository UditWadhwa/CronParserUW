...
public class DayOfMonthSlashHandler extends BaseSlashHandler implements SlashHandler {
    ...
    protected void validate(String termStr, Term term) throws Exception {
        ...
        if(beginValue < 0 || beginValue > 31 || stepRangeValue < 0 || stepRangeValue > 31)
            throw new NumericOutOfRangeException(beginValue, stepRangeValue, 1, 31, term);
    }
    ...
    public String process(String termStr, Term term) throws Exception{
        ...
        if(stepRangeValue == 0)
            stepRangeValue =1;
        for(i= beginValue; i<= 31; i+= stepRangeValue){
